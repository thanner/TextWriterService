...
public class EPC extends AbstractDirectedGraph<ControlFlow, FlowObject> implements IEPC<ControlFlow,
    ...
    public ControlFlow addControlFlow(FlowObject from, FlowObject to, float probability) {
        ...
        Collection<FlowObject> ss = new ArrayList<FlowObject>();
        ss.add(from);
        Collection<FlowObject> ts = new ArrayList<FlowObject>();
        ts.add(to);
    ...
    public Node addNode(Node obj) {
        if (obj instanceof FlowObject) {
            return this.addFlowObject((FlowObject) obj);
        } else if (obj instanceof NonFlowObject) {
            return this.addNonFlowObject((NonFlowObject) obj);
        }
        return null;
    }
    ...
    public Connection connectNonFlowObject(NonFlowObject obj, FlowObject to) {
        ...
        Set<Connection> h1 = new HashSet<Connection>();
        h1.add(cxn);
        if (this.nf.get(obj) == null) {
            this.nf.put(obj, h1);
            result = true;
        } else result |= this.nf.get(obj).add(cxn);
        Set<Connection> h2 = new HashSet<Connection>();
        h2.add(cxn);
        if (this.fn.get(to) == null) {
            this.fn.put(to, h2);
            result = true;
    ...
    public boolean isJoin(Connector c) {
        return this.getIncomingControlFlow(c).size() > 1 && this.getOutgoingControlFlow(c).size() == 1;
