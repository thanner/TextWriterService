...
import processToText.dataModel.p2t.WFnet2Processes;
import processToText.dataModel.petri.Process;
import processToText.dataModel.petri.ProcessCover;
...
public class PlanningHelper {
    ...
    public static boolean isEndEvent(Object o, ProcessModel process) {
        if (o.getClass().toString().equals("class de.hpi.bpt.process.Event") == true) {
            processToText.dataModel.process.Event event = process.getEvents().get(
    ...
    public static ArrayList<ArrayList<String>> getRunSequencesFromRPSTFragment(
        ...
        for (AbstractDirectedEdge arc : node.getFragment()
                .getEdges()) {
        ...
        for (Process p : fps.getCorrectProcesses()) {
            c++;
        ...
        for (RPSTNode<ControlFlow, Node> node : orderedTopNodes) {
            ...
            if (PlanningHelper.isBond(node)) {
                ...
                if (PlanningHelper.isTask(node.getEntry())) {
                    printIndent(level);
                    Activity activity = process.getActivity(Integer
